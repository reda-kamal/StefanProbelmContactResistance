...
%PLOT_FLUX Plot interface heat flux histories for VAM and numerical solutions.
    if nargin<3 || isempty(t_max)
        snap = get_explicit_snapshot(caseX);
        if ~isempty(snap) && isfield(snap, 't')
            t_max = max(0.1, snap.t);
        else
            t_max = 0.1;
        end
    end
    ...
    snap_exp = get_explicit_snapshot(caseX);
    if ~isempty(snap_exp)
...
function snap = get_explicit_snapshot(caseX)
    snap = [];
    if ~isfield(caseX,'num') || isempty(caseX.num)
        return;
    end
    num_struct = caseX.num;
    if ~isstruct(num_struct)
        return;
    end
